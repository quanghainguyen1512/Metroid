...
RidleyBullet::RidleyBullet(LPD3DXSPRITE spriteHandler, Grid*grid) {
	...
	this->height = HEIGHT_RIDLEY_BULLET + 10;
}
...
void RidleyBullet::initBullet(float posX, float posY) {
...
void RidleyBullet::Render() {
	...
	this->ridleyBullet->drawSprite(0, 0, WIDTH_RIDLEY_BULLET, HEIGHT_RIDLEY_BULLET, pos);
}

void RidleyBullet::InitSprites(LPDIRECT3DDEVICE9 d3ddv, LPDIRECT3DTEXTURE9 texture) {
	if (d3ddv == NULL) return;
	ridleyBullet = new Sprite(this->spriteHandler, texture, RIDLEY_BULLET_PATH, WIDTH_RIDLEY_BULLET, HEIGHT_RIDLEY_BULLET, RIDLEY_BULLET_COUNT);
}
...
void RidleyBullet::setIsRendered(bool isRendered) {
	this->isRendered = isRendered;
}

bool RidleyBullet::getIsRendered() {
	return this->isRendered;
}

void RidleyBullet::setCount(int count) {
	this->count = count;
}

int RidleyBullet::getCount() {
	return this->count;
}